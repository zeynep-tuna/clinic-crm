...
interface SecretaryAppointmentFormState {
  patientId: string;
  doctorId: string;
  date: string;
  time: string;
...
const initialFormState: SecretaryAppointmentFormState = {
  patientId: "",
  doctorId: "",
  date: "",
  time: "",
...
type SecretaryAppointmentFormErrors = Partial<Record<keyof SecretaryAppointmentFormState, string>>;

function validateSecretaryAppointmentForm(
  values: SecretaryAppointmentFormState
): SecretaryAppointmentFormErrors {
  const nextErrors: SecretaryAppointmentFormErrors = {};
  ...
  if (!values.status) {
    nextErrors.status = "Randevu durumu zorunludur.";
  }
...
interface AddSecretaryAppointmentModalProps {
  onCreated?: () => void;
}

export default function AddSecretaryAppointmentModal({ onCreated }: AddSecretaryAppointmentModalProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [form, setForm] = useState<SecretaryAppointmentFormState>(initialFormState);
  const [errors, setErrors] = useState<SecretaryAppointmentFormErrors>({});
  ...
  function updateField<K extends keyof SecretaryAppointmentFormState>(
    key: K,
    value: SecretaryAppointmentFormState[K]
  ) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const validationErrors = validateSecretaryAppointmentForm(form);
      ...
      {isOpen && (
        ...
        >
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="add-secretary-appointment-modal-title"
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <h2 id="add-secretary-appointment-modal-title" className="text-xl font-bold text-[#0B1F55]">
            ...
            <form onSubmit={handleSubmit} className="mt-6 grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-2">
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium text-[#0B1F55]">İşlem / Tedavi Türü</label>
                <select
              ...
              <div className="sm:col-span-2">
                <label className="mb-1.5 block text-sm font-medium text-[#0B1F55]">Not</label>
                <textarea
                  value={form.notes}
