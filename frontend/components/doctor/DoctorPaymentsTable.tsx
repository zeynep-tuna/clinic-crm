...
const statusBadgeClass: Record<DoctorPaymentStatus, string> = {
  Ödendi: "bg-[#DCFCE7] text-[#16A34A]",
  Bekliyor: "bg-[#FEF3C7] text-[#F59E0B]",
  "Kısmi Ödeme": "bg-[#DBEAFE] text-[#2563EB]",
...
export default function DoctorPaymentsTable() {
  ...
  return (
    <div className="flex flex-col gap-5">
      <DoctorPaymentSummaryCards activeFilter={activeFilter} onFilterChange={setActiveFilter} />
            ...
            <tbody>
              {filteredPayments.map((payment) => (
                ...
                >
                  ...
                  <td className="py-4 text-sm font-bold text-[#0B1F55]">{payment.amount}</td>
                  <td className="py-4">
                    <span className="inline-block rounded-full border border-[#E3E8F0] bg-[#F7F8FF] px-3 py-1 text-xs font-medium text-[#0B1F55]">
                      {payment.paymentMethod}
                    </span>
                  </td>
                  <td className="py-4 text-sm text-[#0B1F55]">{payment.date}</td>
                  <td className="py-4">
                    <span
                      className={`inline-block rounded-full px-3.5 py-1.5 text-xs font-semibold ${statusBadgeClass[payment.status]}`}
                    >
                      {payment.status}
                    </span>
                  </td>
