...
export default function PaymentsTable() {
  ...
  const filteredPayments = useMemo(() => {
    ...
    return payments.filter((payment) => {
      ...
      const matchesSearch = term === "" || payment.patientName.toLowerCase().includes(term);
        ...
        <table className="w-full min-w-[960px] table-fixed text-left">
          <thead>
            <tr className="border-b border-[#E3E8F0] text-sm text-[#667085]">
              ...
              <th className="w-44 pb-3 pr-4 font-medium">Tedavi</th>
              <th className="w-32 pb-3 pr-4 font-medium">Tutar</th>
              <th className="w-36 pb-3 pr-4 font-medium">Ödeme Durumu</th>
              <th className="w-36 pb-3 pr-4 font-medium">Ödeme Yöntemi</th>
          ...
          <tbody>
            {filteredPayments.map((payment) => (
              <tr key={payment.id} className="border-b border-[#E3E8F0]/60 last:border-0">
                ...
                <td className="truncate py-5 pr-4 text-sm text-[#0B1F55]">{payment.treatment}</td>
                ...
                <td className="truncate py-5 pr-4 text-sm text-[#0B1F55]">{payment.method}</td>
                <td className="py-5 pr-4 text-sm text-[#0B1F55]">{payment.date}</td>
                <td className="py-5">
