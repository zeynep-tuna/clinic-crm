...
function getStatusLabel(doctor: Doctor): UiStatus {
  return doctor.isActive ? "Aktif" : "Pasif";
}
...
export default function DoctorsTable({ refreshKey }: DoctorsTableProps) {
  ...
  const loadDoctors = useCallback(async () => {
    ...
    try {
      const data = await listDoctors({ includeInactive: true });
      setDoctorList(data);
    } catch {
  ...
  const filteredDoctors = useMemo(() => {
    ...
    return doctorList.filter((doctor) => {
      const status = getStatusLabel(doctor);
    ...
    <div className="flex flex-col gap-5">
      {isLoading && (
        <div className="rounded-[20px] border border-[#EAF0F8] bg-white p-10 text-center shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
          <p className="text-sm text-[#667085]">Doktorlar yükleniyor...</p>
        </div>
      )}
                ...
                <tbody>
                  {filteredDoctors.map((doctor) => {
                    ...
                    >
                      ...
                      <td className="truncate py-4 pr-4 text-sm text-[#0B1F55]" title={doctor.specialty ?? undefined}>
                        {formatNullable(doctor.specialty)}
                      </td>
